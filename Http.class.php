...
class Http {
    ...
    static public function get_file_content($url) {
        //使用curl来抓去url结果
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);  //设置抓取地址
        curl_setopt($ch, CURLOPT_HEADER, 0);  //设置不要头信息
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);  //设置curl_exec输出到变量
        $content = curl_exec($ch);
        curl_close($ch);
        return $content;
    }

    static public function curlMallApi($method, $data) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, C("MALL_API_HOST") . "/api.php?method=" . $method);
    ...
    static public function curlSceneApi($data) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, C("SCENE_API_HOST") . "/api");
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($ch, CURLOPT_AUTOREFERER, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
    ...
    public static function curlTicketApi($apiUrl, $data) {
        $params = "入参----" . var_export($data, TRUE);
        Log::jobLog($params);
        $ch = curl_init();
        ...
        $params = "返回值----" . var_export($ret, TRUE);
        Log::jobLog($params);
        return $ret;
    }

    public static function curlWifiApi($apiUrl) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $apiUrl);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $output = curl_exec($ch);
    ...
    static function format_params($parms) {
        $post_url = "";
        if (!empty($parms)) {
